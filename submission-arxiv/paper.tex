...
\textbf{The estimators earn their place.} The same ratings aggregated the plain way, every portfolio's leave-self-out mean over all axes and judges, track GPQA at \(r = 0.77\) un-anchored (the \S{}4.1 pass) and \(0.93\) anchored (run 1, the anchor at 7 by calibration), against \(0.98\) for \(T\) (\texttt{scripts/baseline\_\allowbreak{}aggregators.py}). Anchoring and the two estimators each account for part of the agreement.
