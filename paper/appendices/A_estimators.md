...
**Intervals.** Every rating carries a 95% percentile bootstrap ($10^3$–$10^4$ replicates). Units: $G$ — the per-(submission, archetype) units; $f$ — the $N$ parallel contexts, each replicate's top-2 left subspace Procrustes-aligned to the full sample before the leading loading is read (the leading axis is near-degenerate between the Anthropic and Google blocs); $\bar r$ — the 55 (submission, archetype) atoms of the shared grid. $E$ and $T$ are **not** combined analytically: they are bootstrapped jointly on the 55-atom grid, leave-self-out applied after the resample, every component including $f_a,\bar r_a$ recomputed on each replicate. The council is held at its selected membership rather than re-selected inside each replicate (the gate is itself defined by a bootstrap interval), so the interval is conditional on that selection.
