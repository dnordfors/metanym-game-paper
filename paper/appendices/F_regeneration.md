...
![The three runs apart and pooled, against GPQA Diamond. Top row: council-basis $T$ per run, with each run's twelve-evaluator spectral gap. Bottom row: runs 1 and 2 pooled (left), all three pooled (right). Run 3, with the smallest gap, is the odd one out; pooling restores an identified factual axis and the agreement rises.](../figures/runs_panel.png)
