...
**The 0.98, and what it licenses.** The total tracks GPQA at 0.98 (§4.5). GPQA is an accepted measure of language-model capability with no theory of intelligence behind it: expert-written questions and a key (Rein et al., 2023). The game reproduces its ranking as closely as the measurement error of the two instruments allows (Appendix D.1), so whatever GPQA measures, the game measures too. The game is a single activity, making analogies and judging them, and of the eight constructs it demands (Appendix E), four are analogy under cognitive science's own names — relational reasoning, structure-mapping, essence-seeing, theory formation by analogy — and the other four are recruited in their service. So a test whose core is analogy reaches all of what GPQA measures: that is the sense in which analogy is at the core — sufficient by itself, not the only thing that could. For the strength of the agreement we offer a hypothesis. Training a language model is compression (Delétang et al., 2024), and compression generalises: the shortest description labels each recurring pattern once and reuses the label (Rissanen, 1978), which is forming a concept. A concept is a general frame plus the slots that differ (Minsky, 1974; Rosch, 1978); an analogy, a shared relational structure with differing particulars (Gentner, 1983). Our hypothesis is that the shared structure is an archetypal context; that a model holds its knowledge as such contexts, each instantiated in the domains where it applies; and that the context templates of §2 are these contexts written out, the metanym sets the differences. If knowledge is held in this form, a GPQA question is read by mapping it onto the archetypal contexts it instantiates and the domain that instantiates them, and the two tests agree because they exercise one operation. The same hypothesis says why the models play well: in the game they do not build templates and metanym sets, they retrieve what training already compressed — consistent with play at temperature zero and no reasoning channel, and visible in the transcripts: between the two runs made two hours apart a model rewrites its portfolios but keeps its archetypes (34 of 48 titles recur verbatim), and seven of eleven models, from all three vendors, offer a resource-allocation archetype. The data establish less: the factual pair alone reaches 0.94, and the two subjective quarters lift it to 0.98 (§4.5). The hypothesis is consistent with the general factor Ilić and Gignac (2024) find across 591 models and twelve tests, and more specific: it predicts that the agreement is domain-matched — a model's factual score on parallel contexts in a field tracks its GPQA accuracy in that field — and that archetype and topic domain are separable in a model's latent space. Every rating and response is released.
