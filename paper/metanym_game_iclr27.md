...
We present evidence that analogy is at the core of LLM intelligence: a benchmark made of nothing but analogy-making and its judgement reproduces GPQA Diamond's ranking of twelve models at Pearson $r = 0.98$ (95% CI [0.95, 0.99]) over three pooled runs, and no leakage could be found. In our Metanym Game Benchmark, LLMs compete in generating sets of analogous statements — one context template instantiated in five distant domains by swapping in metanym sets — and rate each other's sets on their own understandings of factual correctness, beauty, intelligence, distinctness, length and structural diversity. Nothing enters from outside: every item is generated in play, and the scores come from the players' ratings alone. Ground truth is replaced by the SVD of the factual rating matrix, whose left and right singular vectors rate the players as judges and as generators, two distinct ratings from one factorisation — to our knowledge a first for an LLM council of peers. For subjective criteria, judges are weighted by their rating consistency under a swept calibration anchor. Generating and judging are different skills: some strong generators are average judges. A council of the five best issues the official ratings; its contestable seats let the benchmark scale to any number of players and rise with the models it measures — a candidate steering signal for self-improving AI. For the agreement with a keyed test that shares nothing with the game on the surface, we offer a hypothesis: a language model holds its knowledge as archetypal contexts instantiated in topic domains; the game measures that organisation directly, GPQA its use. Every number recomputes from a released package.
