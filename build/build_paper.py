...
def postfix(body: str) -> str:
    ...
    body = re.sub(r"\\begin\{figure\}\[t\](\n\\centering\n(?:\\pandocbounded\{)?\\includegraphics\[[^\]]*\]\{figures/(?:game_(?:example|generation|evaluation)|council_evaluation_pc1_all)\.png)", r"\\begin{figure}[p]\1", body)   # Figure 1: its own page
    body = re.sub(r"(\\includegraphics\[[^\]]*\]\{figures/(?:game_(?:example|generation|evaluation)|council_evaluation_pc1_all)\.png\}\}?)\n(\\caption\{)", r"\1\n\\vspace{-14pt}\n\2", body)   # Figure 1: caption tight under the exhibit
...
def arxiv_edits(md: str) -> str:
    ...
    two = ('[]{#fig-game-generation}\n\n![The Metanym Game, generation: the archetypal context \u2018Gradient-Guided Navigation\u2019 from the anchor submission; its context template, metanym table (one metanym set excluded to make space in the figure) and one instantiation with its idiomatic rewrite.](figures/game_generation.png)\n\n'
           '[]{#fig-game-evaluation}\n\n![The Metanym Game, evaluation: the council on one parallel context of the Gemini 2.5 Flash submission; the instantiation, its idiomatic rewrite, the administrator\u2019s summary, and the five judges\u2019 ratings with their justifications.](figures/council_evaluation_pc1_all.png)\n')
