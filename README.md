...
[10.48550/arXiv.2606.21008](https://doi.org/10.48550/arXiv.2606.21008)), September 2026; the version
submitted to ICLR 2027. **Author:** David Nordfors (david.nordfors@archetypes.ai).
...
> **Review status.** While the paper is under double-blind review this repository must not be linked
> from the submission; reviewers receive an anonymised copy.
...
build/
  build_paper.py              markdown -> paper.tex -> PDF, in the ICLR 2027 style
...
python3 build/build_paper.py             # the ICLR submission: anonymised PDF and bundle, written outside this tree
```

Requires `pandoc` and `tectonic`. The ICLR build fails loudly if the main text runs past the 9-page
limit or if an author name, repository URL or arXiv id survives into the anonymised PDF.

## The versions

v3 (this tree) pools the three full runs of the game — one factorisation over all three runs' factual
scores, each player's three portfolios graded — and is the version submitted to ICLR 2027. v2, in
