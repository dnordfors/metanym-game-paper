...
DATA = Path(__file__).resolve().parent.parent / "data"; PKGD = Path(__file__).resolve().parent.parent / "data"; TAG = __import__("os").environ.get("TAG", "pooled123")


def rdp(name):
    return list(csv.DictReader([l for l in (PKGD / name).read_text().splitlines()
...
council = {r["model"]: r for r in rd(f"total_rating_council_{TAG}.csv")}
twelve = {r["model"]: r for r in rd(f"total_rating_twelve_{TAG}.csv")}
runs = {r["model"]: r for r in rdp("total_rating_runs.csv")}
ecsvd = {r["model"]: float(r["EC_svd"]) for r in rdp("ec_svd_twelve.csv")}
gpqa = {r["model"]: float(r["gpqa_diamond_accuracy"]) for r in rdp("gpqa_selfadministered.csv")}
ext = {r["model"]: r["gpqa_diamond"] for r in rdp("external_benchmarks.csv")}
...
print(f"headline: T r={rs['T']:.3f}, G^C r={rs['G^C alone']:.3f} (pooled: no assertion)")
