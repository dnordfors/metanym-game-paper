...
The test is the \textbf{metanym game}. A player authors, from nothing, a \emph{context template} --- a paragraph of fixed wording with open slots --- together with the sets of keywords that fill it, each set instantiating the template as a factually true description of a different domain; the keywords in corresponding slots are \emph{metanyms}, metaphorically synonymous, and a set of them a \emph{metanym set}. Take a passage on \textbf{cell signalling}, adapted from an old version of the Wikipedia article: ``CELL SIGNALING is part of a complex system of communication that governs basic CELLULAR activities and coordinates CELL actions. The ability of CELLS to perceive and correctly respond to their MICROENVIRONMENT is the basis of development, TISSUE repair, and IMMUNITY as well as normal TISSUE HOMEOSTASIS.'' Substitute the marked keywords and it becomes a correct description of \textbf{human language}: ``HUMAN LANGUAGE is part of a complex system of communication that governs basic HUMAN activities and coordinates HUMAN actions. The ability of HUMANS to perceive and correctly respond to their ENVIRONMENT is the basis of development, COMMUNITY repair, and RESILIENCE as well as normal COMMUNITY EQUILIBRIUM.'' Each sentence is checkable on its own --- factually true even where the borrowed phrasing reads stiffly. The two are each other's \emph{metaphors}; the instantiations are \emph{parallel contexts}, children of a common \emph{archetypal context} --- the abstract structure they share, of which the template is the literal representation. A long tradition treats seeing one structure across wildly different domains as central to thought and tests whether you \emph{recognise} it; the game tests whether you can \emph{build} it.
...
Each parallel context is played in two forms: the \textbf{instantiation} (Form a), the mechanical substitution --- only the slots filled, every other word carried over --- which is what the factual grading acts on, because it must come out true sentence by sentence; and the \textbf{idiomatic rewrite} (Form b) in the target domain's own register, showing the claim is not an artefact of the template's phrasing. One template, mechanically swappable metanyms, true sentence by sentence across maximal domain distance: that is what makes a metanym game decidable, and therefore measurable.

\subsection{The rules}\label{the-rules}

The game has \(N\) players and a non-competing administrator. \textbf{Generation}: a player creates archetypal contexts from scratch --- a portfolio of \(K\) templates, \(M\) metanym sets each (five and five here), with instantiation and rewrite for every set. \textbf{Evaluation}: a player scores other players' submissions on the rubric axes (\S{}3.2) against one fixed \emph{reference} submission pinned at an \emph{anchor} value, the anchor swept across declared values (\{5, 6, 7, 8\} here) --- the only thing that changes between passes. One pass yields \textbf{submission ratings} for each portfolio and \textbf{evaluator ratings} for the judges: how well one detects the factual errors the other players collectively flag (\emph{factual competence}), and how stable a standard it holds as the anchor shifts (\emph{rating consistency}). Each act is itself rated, so the framework is \textbf{fully self-contained}: no human raters, no external key.
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.11\linewidth}>{\raggedright\arraybackslash}p{0.57\linewidth}}
\toprule\noalign{}
...
\textbf{The council.} The \textbf{council} is the five players with the highest total \(T\) (eq. 2) and issues every official rating. The top five are the better factual judges (loadings 0.26--0.61, \S{}4.2); the sixth reads 0.18; the line is drawn there, and nothing prescribes five: the council grows as competent judges arrive.

\textbf{The total.} Each side of the game splits into a factual and a criterion half: on generation, \(G^{F}\) (the SVD generation factuality) and \(G^{C}\) (the council's leave-self-out mean over the five non-factual axes, each seat's vote weighted by its own consistency on that axis); on evaluation, \(E^{F} = 7f/f_a\) and \(E^{C} = 7\bar r/\bar r_a\) --- \textbf{the anchor model scores 7} on every component. The total is the mean of the four, a symmetric \(2\times2\) of \{generator, evaluator\} \ensuremath{\times} \{factual, criterion\}:
...
\textbf{Scaling by addition.} A standing council scores any future model against the same anchor without re-deriving existing ratings. The seats are \textbf{contestable} --- a contestant submits a portfolio, evaluates the incumbents' portfolios and is scored by the seats under the definitions above, and wins a seat with a total \(T\) above the lowest seat's by a margin the bootstrap can resolve. Because a contest convenes only the top of the field, two fixed \textbf{ballast} blocks --- the weakest archived portfolios --- join every contest's graded set to keep the factual axis identified (Appendix A.6), and two guards: the spectral gap \(\sigma_1/\sigma_2 \ge 2\) --- the shared judgement standing clear of the strongest disagreement, sized in Appendix A.6 --- and a spread of the seats' \(E^{F}\) above 2.5 points. If either fails, no seat changes hands and the contest's totals carry a caveat. The official leaderboard (\S{}4.4) is issued on this contest basis. \textbf{Contamination}: items are generated fresh each run, so no fixed test set can leak into training. Every submission is archived; a new portfolio that copies an archived one is identified, and the contestant is asked for new templates --- or the game is played in another language.

\section{Results}\label{results}

\subsection{Anchoring doubles resolution}\label{anchoring-doubles-resolution}

The bootstrap opens with a raw pass --- every portfolio scored by every other model with no anchor, averaged leave-self-out, 95\% bootstrap intervals (2,000 resamples; Efron \& Tibshirani, 1993). It supplies the baseline: the top-ranked portfolio, \textbf{claude-opus-4.5}'s, whose first archetype is Table \ref{tab-anchor-metanym}, is pinned at 7 on every axis, leaving headroom above. Re-run anchored (Appendix G), the gap between a leading eight and a trailing four more than doubles relative to the spread of the means --- every cross-band pair holds at bootstrap probability 1.00 --- while ranks within either band stay unresolved.
...
The anchor sweep gives each evaluator a consistency on each axis (Table \ref{tab-criterion-b}, Appendix A.3). Collapse is either \emph{uniform} --- no stable signal on any axis --- or \emph{axis-specific}: gemini-2.5-flash (0.31) collapses on factual while its other axes hold. The measure is self-consistency, not accuracy. On the five subjective criteria generation and evaluation align (anchored cosine 0.85--0.92 per criterion, Appendix A.5) --- the counterpoint to the factual axis, where they come apart (\S{}4.4).
...
\begin{center}\small
\resizebox{\linewidth}{!}{\begin{tabular}{@{}llcrrrrrrr@{}}
\toprule\noalign{}
Rank & Model & Council & \textbf{\(T\) {[}95\% CI{]}} & \(E\) & \(G\) & \(G^{F}\) & \(G^{C}\) & \(E^{F}\) & \(E^{C}\) \\
\midrule\noalign{}
1 & \ensuremath{\star} claude-opus-4.5 (anchor) & council &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{\textbf{7.00 {[}7.00, 7.00{]}}}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00} \\
2 & gemini-3.1-pro & council &\cellcolor[HTML]{2076B3}\textcolor[HTML]{FFFFFF}{\textbf{6.92 {[}6.69, 7.24{]}}}&\cellcolor[HTML]{2262AA}\textcolor[HTML]{FFFFFF}{7.39}&\cellcolor[HTML]{1E89BC}\textcolor[HTML]{FFFFFF}{6.45}&\cellcolor[HTML]{1F7CB6}\textcolor[HTML]{FFFFFF}{6.77}&\cellcolor[HTML]{2095C0}\textcolor[HTML]{FFFFFF}{6.13}&\cellcolor[HTML]{2355A4}\textcolor[HTML]{FFFFFF}{7.78}&\cellcolor[HTML]{2072B1}\textcolor[HTML]{FFFFFF}{7.01} \\
3 & claude-opus-4.1 & council &\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{\textbf{6.02 {[}5.71, 6.32{]}}}&\cellcolor[HTML]{3FB4C4}\textcolor[HTML]{000000}{5.06}&\cellcolor[HTML]{2073B2}\textcolor[HTML]{FFFFFF}{6.98}&\cellcolor[HTML]{2075B3}\textcolor[HTML]{FFFFFF}{6.94}&\cellcolor[HTML]{2072B1}\textcolor[HTML]{FFFFFF}{7.02}&\cellcolor[HTML]{85CFBA}\textcolor[HTML]{000000}{3.65}&\cellcolor[HTML]{1E88BC}\textcolor[HTML]{FFFFFF}{6.47} \\
4 & claude-opus-4.0 & council &\cellcolor[HTML]{2A9EC1}\textcolor[HTML]{FFFFFF}{\textbf{5.81 {[}5.47, 6.12{]}}}&\cellcolor[HTML]{43B7C4}\textcolor[HTML]{000000}{4.95}&\cellcolor[HTML]{1F7FB8}\textcolor[HTML]{FFFFFF}{6.68}&\cellcolor[HTML]{2076B3}\textcolor[HTML]{FFFFFF}{6.91}&\cellcolor[HTML]{1E89BC}\textcolor[HTML]{FFFFFF}{6.45}&\cellcolor[HTML]{8ED3BA}\textcolor[HTML]{000000}{3.49}&\cellcolor[HTML]{1E8ABD}\textcolor[HTML]{FFFFFF}{6.41} \\
5 & gemini-2.5-flash & council &\cellcolor[HTML]{2BA0C2}\textcolor[HTML]{FFFFFF}{\textbf{5.75 {[}4.98, 6.21{]}}}&\cellcolor[HTML]{35AAC3}\textcolor[HTML]{FFFFFF}{5.41}&\cellcolor[HTML]{2296C1}\textcolor[HTML]{FFFFFF}{6.08}&\cellcolor[HTML]{1E88BC}\textcolor[HTML]{FFFFFF}{6.46}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.70}&\cellcolor[HTML]{64C3BF}\textcolor[HTML]{000000}{4.30}&\cellcolor[HTML]{1E86BB}\textcolor[HTML]{FFFFFF}{6.52} \\
6 & claude-sonnet-4 & --- &\cellcolor[HTML]{37ACC3}\textcolor[HTML]{FFFFFF}{\textbf{5.34 {[}5.09, 5.65{]}}}&\cellcolor[HTML]{6EC7BE}\textcolor[HTML]{000000}{4.10}&\cellcolor[HTML]{1E84BA}\textcolor[HTML]{FFFFFF}{6.58}&\cellcolor[HTML]{2074B3}\textcolor[HTML]{FFFFFF}{6.95}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.21}&\cellcolor[HTML]{D4EEB3}\textcolor[HTML]{000000}{2.07}&\cellcolor[HTML]{2095C0}\textcolor[HTML]{FFFFFF}{6.13} \\
7 & gpt-4.1-mini & --- &\cellcolor[HTML]{40B5C4}\textcolor[HTML]{000000}{\textbf{5.03 {[}4.37, 5.62{]}}}&\cellcolor[HTML]{63C3BF}\textcolor[HTML]{000000}{4.31}&\cellcolor[HTML]{2CA0C2}\textcolor[HTML]{FFFFFF}{5.74}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.23}&\cellcolor[HTML]{3AAFC3}\textcolor[HTML]{000000}{5.25}&\cellcolor[HTML]{E1F3B2}\textcolor[HTML]{000000}{1.66}&\cellcolor[HTML]{2074B2}\textcolor[HTML]{FFFFFF}{6.97} \\
8 & gpt-4.1-2025-04-14 & --- &\cellcolor[HTML]{52BCC2}\textcolor[HTML]{000000}{\textbf{4.66 {[}4.49, 5.03{]}}}&\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.50}&\cellcolor[HTML]{299EC1}\textcolor[HTML]{FFFFFF}{5.82}&\cellcolor[HTML]{1E83B9}\textcolor[HTML]{FFFFFF}{6.59}&\cellcolor[HTML]{40B5C4}\textcolor[HTML]{000000}{5.04}&\cellcolor[HTML]{F4FBC0}\textcolor[HTML]{000000}{0.77}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.23} \\
9 & gpt-4.1-nano & --- &\cellcolor[HTML]{83CFBB}\textcolor[HTML]{000000}{\textbf{3.68 {[}3.31, 4.05{]}}}&\cellcolor[HTML]{96D6B9}\textcolor[HTML]{000000}{3.35}&\cellcolor[HTML]{72C8BD}\textcolor[HTML]{000000}{4.02}&\cellcolor[HTML]{57BEC1}\textcolor[HTML]{000000}{4.55}&\cellcolor[HTML]{8FD3B9}\textcolor[HTML]{000000}{3.48}&\cellcolor[HTML]{FBFED1}\textcolor[HTML]{000000}{0.26}&\cellcolor[HTML]{1E8ABD}\textcolor[HTML]{FFFFFF}{6.43} \\
10 & gpt-4o-2024-08-06 & --- &\cellcolor[HTML]{97D6B9}\textcolor[HTML]{000000}{\textbf{3.34 {[}3.00, 3.56{]}}}&\cellcolor[HTML]{CCEBB4}\textcolor[HTML]{000000}{2.33}&\cellcolor[HTML]{62C2BF}\textcolor[HTML]{000000}{4.34}&\cellcolor[HTML]{3CB0C3}\textcolor[HTML]{000000}{5.19}&\cellcolor[HTML]{8ED3BA}\textcolor[HTML]{000000}{3.49}&\cellcolor[HTML]{FDFED4}\textcolor[HTML]{000000}{0.15}&\cellcolor[HTML]{59BFC0}\textcolor[HTML]{000000}{4.51} \\
11 & gpt-4o & --- &\cellcolor[HTML]{ADDFB7}\textcolor[HTML]{000000}{\textbf{2.95 {[}2.51, 3.29{]}}}&\cellcolor[HTML]{E4F5B2}\textcolor[HTML]{000000}{1.53}&\cellcolor[HTML]{60C2BF}\textcolor[HTML]{000000}{4.37}&\cellcolor[HTML]{3BAFC3}\textcolor[HTML]{000000}{5.22}&\cellcolor[HTML]{8CD2BA}\textcolor[HTML]{000000}{3.53}&\cellcolor[HTML]{F9FDCB}\textcolor[HTML]{000000}{0.44}&\cellcolor[HTML]{C1E7B5}\textcolor[HTML]{000000}{2.61} \\
12 & gpt-4o-mini & --- &\cellcolor[HTML]{CAEAB4}\textcolor[HTML]{000000}{\textbf{2.39 {[}1.96, 2.79{]}}}&\cellcolor[HTML]{EEF8B4}\textcolor[HTML]{000000}{1.17}&\cellcolor[HTML]{86D0BA}\textcolor[HTML]{000000}{3.62}&\cellcolor[HTML]{81CEBB}\textcolor[HTML]{000000}{3.71}&\cellcolor[HTML]{8CD2BA}\textcolor[HTML]{000000}{3.53}&\cellcolor[HTML]{FFFFD9}\textcolor[HTML]{000000}{-0.00}&\cellcolor[HTML]{CCEBB4}\textcolor[HTML]{000000}{2.34} \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\begin{figure}[t]
\centering
\pandocbounded{\includegraphics[width=0.32\linewidth]{figures/total_validation_simple.png}}
\caption{The official total rating \(T\) (council basis) against self-administered GPQA Diamond accuracy, twelve models. Pearson \(r = 0.98\) (95\% BCa bootstrap CI {[}0.95, 0.99{]}), Spearman \(\rho = 0.96\); three runs pooled. Filled markers are council seats, open markers non-council; horizontal bars the joint-bootstrap 95\% CI on \(T\), vertical bars the GPQA binomial 95\% CI; the line is the least-squares fit. The star is the anchor: \(T = 7\) by calibration, GPQA measured independently; excluding it leaves \(r\) at 0.98.}
\label{fig-gpqa-scatter}
\end{figure}

\(T\) reproduces GPQA's ordering at Pearson \(r = 0.98\) {[}0.95, 0.99{]} (Spearman 0.96; three runs pooled, \S{}4.6). Instruments sharing no item authors, task or scoring, and agreeing at 0.98, are as close as their measurement error allows (Appendix D.1); what they have in common, \S{}6 leaves open. The objection that any two demanding benchmarks correlate on a wide roster does not carry --- the agreement holds at \(r = 0.94\) within the leading eight alone.
...
\begin{center}\small
\resizebox{\linewidth}{!}{\begin{tabular}{@{}lrrlrl@{}}
\toprule\noalign{}
Model & \(E^{F}\) loading & \(E^{F}\) anchored & 95\% CI & \(G^{F}\) & 95\% CI \\
\midrule\noalign{}
gemini-3.1-pro &\cellcolor[HTML]{2195C0}\textcolor[HTML]{FFFFFF}{0.61}&\cellcolor[HTML]{24459C}\textcolor[HTML]{FFFFFF}{8.24}& {[}7.33, 9.30{]} &\cellcolor[HTML]{1F7BB6}\textcolor[HTML]{FFFFFF}{6.78}& {[}6.62, 6.89{]}  \\
claude-opus-4.5 &\cellcolor[HTML]{3BB0C3}\textcolor[HTML]{000000}{0.52}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}& {[}7.00, 7.00{]} &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}& (anchor)  \\
claude-opus-4.0 &\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{0.35}&\cellcolor[HTML]{50BCC2}\textcolor[HTML]{000000}{4.69}& {[}3.45, 6.04{]} &\cellcolor[HTML]{2075B3}\textcolor[HTML]{FFFFFF}{6.93}& {[}6.89, 6.97{]}  \\
claude-opus-4.1 &\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{0.35}&\cellcolor[HTML]{52BCC1}\textcolor[HTML]{000000}{4.65}& {[}3.56, 5.96{]} &\cellcolor[HTML]{2073B2}\textcolor[HTML]{FFFFFF}{6.99}& {[}6.91, 7.07{]}  \\
gemini-2.5-flash &\cellcolor[HTML]{C1E7B5}\textcolor[HTML]{000000}{0.26}&\cellcolor[HTML]{8FD3B9}\textcolor[HTML]{000000}{3.47}& {[}1.62, 5.05{]} &\cellcolor[HTML]{1E87BB}\textcolor[HTML]{FFFFFF}{6.49}& {[}6.27, 6.68{]}  \\
claude-sonnet-4 &\cellcolor[HTML]{DCF1B2}\textcolor[HTML]{000000}{0.18}&\cellcolor[HTML]{CBEBB4}\textcolor[HTML]{000000}{2.37}& {[}1.53, 3.36{]} &\cellcolor[HTML]{2074B2}\textcolor[HTML]{FFFFFF}{6.96}& {[}6.92, 7.00{]}  \\
gpt-4.1-mini &\cellcolor[HTML]{F1F9B9}\textcolor[HTML]{000000}{0.10}&\cellcolor[HTML]{EAF7B1}\textcolor[HTML]{000000}{1.34\ensuremath{\dagger}}& --- &\cellcolor[HTML]{1D91C0}\textcolor[HTML]{FFFFFF}{6.26}& {[}6.07, 6.43{]}  \\
gpt-4.1-2025-04-14 &\cellcolor[HTML]{F6FCC6}\textcolor[HTML]{000000}{0.06}&\cellcolor[HTML]{F4FBC0}\textcolor[HTML]{000000}{0.79}& {[}0.39, 1.53{]} &\cellcolor[HTML]{1E84BA}\textcolor[HTML]{FFFFFF}{6.56}& {[}6.39, 6.69{]}  \\
gpt-4o &\cellcolor[HTML]{F9FDCC}\textcolor[HTML]{000000}{0.04}&\cellcolor[HTML]{F8FCCA}\textcolor[HTML]{000000}{0.47}& {[}0.28, 0.70{]} &\cellcolor[HTML]{37ACC3}\textcolor[HTML]{FFFFFF}{5.33}& {[}5.12, 5.53{]}  \\
gpt-4o-2024-08-06 &\cellcolor[HTML]{FBFDCF}\textcolor[HTML]{000000}{0.03}&\cellcolor[HTML]{FAFDCE}\textcolor[HTML]{000000}{0.35}& {[}0.18, 0.61{]} &\cellcolor[HTML]{36AAC3}\textcolor[HTML]{FFFFFF}{5.39}& {[}5.16, 5.62{]}  \\
gpt-4.1-nano &\cellcolor[HTML]{FCFED3}\textcolor[HTML]{000000}{0.02}&\cellcolor[HTML]{FBFDD0}\textcolor[HTML]{000000}{0.28\ensuremath{\dagger}}& --- &\cellcolor[HTML]{4AB9C3}\textcolor[HTML]{000000}{4.82}& {[}4.01, 5.38{]}  \\
gpt-4o-mini &\cellcolor[HTML]{FFFFD9}\textcolor[HTML]{000000}{-0.00}&\cellcolor[HTML]{FFFFD9}\textcolor[HTML]{000000}{-0.00\ensuremath{\dagger}}& --- &\cellcolor[HTML]{6CC6BE}\textcolor[HTML]{000000}{4.14}& {[}3.60, 4.69{]}  \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\midrule\noalign{}
gemini-3.1-pro &\cellcolor[HTML]{243392}\textcolor[HTML]{FFFFFF}{0.88}&\cellcolor[HTML]{24439B}\textcolor[HTML]{FFFFFF}{0.83}&\cellcolor[HTML]{1F2F88}\textcolor[HTML]{FFFFFF}{0.90}&\cellcolor[HTML]{253695}\textcolor[HTML]{FFFFFF}{0.87}&\cellcolor[HTML]{1F2F88}\textcolor[HTML]{FFFFFF}{0.90}&\cellcolor[HTML]{22318D}\textcolor[HTML]{FFFFFF}{0.89} \\
claude-opus-4.5 &\cellcolor[HTML]{1F2F88}\textcolor[HTML]{FFFFFF}{0.90}&\cellcolor[HTML]{253996}\textcolor[HTML]{FFFFFF}{0.86}&\cellcolor[HTML]{243C98}\textcolor[HTML]{FFFFFF}{0.85}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}&\cellcolor[HTML]{1D2E83}\textcolor[HTML]{FFFFFF}{0.91}&\cellcolor[HTML]{243C98}\textcolor[HTML]{FFFFFF}{0.85} \\
claude-opus-4.1 &\cellcolor[HTML]{24439B}\textcolor[HTML]{FFFFFF}{0.83}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}&\cellcolor[HTML]{243C98}\textcolor[HTML]{FFFFFF}{0.85}&\cellcolor[HTML]{216AAE}\textcolor[HTML]{FFFFFF}{0.72}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}&\cellcolor[HTML]{1B2C7E}\textcolor[HTML]{FFFFFF}{0.92} \\
claude-opus-4.0 &\cellcolor[HTML]{2354A3}\textcolor[HTML]{FFFFFF}{0.78}&\cellcolor[HTML]{24439B}\textcolor[HTML]{FFFFFF}{0.83}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}&\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{0.69}&\cellcolor[HTML]{24439B}\textcolor[HTML]{FFFFFF}{0.83}&\cellcolor[HTML]{253996}\textcolor[HTML]{FFFFFF}{0.86} \\
gpt-4.1-mini &\cellcolor[HTML]{253695}\textcolor[HTML]{FFFFFF}{0.87}&\cellcolor[HTML]{2257A5}\textcolor[HTML]{FFFFFF}{0.77}&\cellcolor[HTML]{225EA8}\textcolor[HTML]{FFFFFF}{0.75}&\cellcolor[HTML]{225BA6}\textcolor[HTML]{FFFFFF}{0.76}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}&\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{0.81} \\
claude-sonnet-4 &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{0.70}&\cellcolor[HTML]{225EA8}\textcolor[HTML]{FFFFFF}{0.75}&\cellcolor[HTML]{2257A5}\textcolor[HTML]{FFFFFF}{0.77}&\cellcolor[HTML]{225EA8}\textcolor[HTML]{FFFFFF}{0.75}&\cellcolor[HTML]{2354A3}\textcolor[HTML]{FFFFFF}{0.78}&\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{0.81} \\
gpt-4.1-2025-04-14 &\cellcolor[HTML]{CAEAB4}\textcolor[HTML]{000000}{0.24}&\cellcolor[HTML]{1E8BBD}\textcolor[HTML]{FFFFFF}{0.64}&\cellcolor[HTML]{2166AC}\textcolor[HTML]{FFFFFF}{0.73}&\cellcolor[HTML]{2262AA}\textcolor[HTML]{FFFFFF}{0.74}&\cellcolor[HTML]{24469D}\textcolor[HTML]{FFFFFF}{0.82}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84} \\
gemini-2.5-flash &\cellcolor[HTML]{A4DCB7}\textcolor[HTML]{000000}{0.31}&\cellcolor[HTML]{2257A5}\textcolor[HTML]{FFFFFF}{0.77}&\cellcolor[HTML]{1E83B9}\textcolor[HTML]{FFFFFF}{0.66}&\cellcolor[HTML]{41B6C4}\textcolor[HTML]{000000}{0.50}&\cellcolor[HTML]{24469D}\textcolor[HTML]{FFFFFF}{0.82}&\cellcolor[HTML]{243C98}\textcolor[HTML]{FFFFFF}{0.85} \\
gpt-4.1-nano &\cellcolor[HTML]{1E87BB}\textcolor[HTML]{FFFFFF}{0.65}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{0.63}&\cellcolor[HTML]{1E8BBD}\textcolor[HTML]{FFFFFF}{0.64}&\cellcolor[HTML]{5FC1C0}\textcolor[HTML]{000000}{0.44}&\cellcolor[HTML]{82CEBB}\textcolor[HTML]{000000}{0.37}&\cellcolor[HTML]{88D0BA}\textcolor[HTML]{000000}{0.36} \\
gpt-4o-2024-08-06 &\cellcolor[HTML]{279BC1}\textcolor[HTML]{FFFFFF}{0.59}&\cellcolor[HTML]{2A9EC1}\textcolor[HTML]{FFFFFF}{0.58}&\cellcolor[HTML]{33A7C2}\textcolor[HTML]{FFFFFF}{0.55}&\cellcolor[HTML]{7DCCBB}\textcolor[HTML]{000000}{0.38}&\cellcolor[HTML]{38ADC3}\textcolor[HTML]{000000}{0.53}&\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{0.69} \\
gpt-4o &\cellcolor[HTML]{EBF7B1}\textcolor[HTML]{000000}{0.13}&\cellcolor[HTML]{9FD9B8}\textcolor[HTML]{000000}{0.32}&\cellcolor[HTML]{CAEAB4}\textcolor[HTML]{000000}{0.24}&\cellcolor[HTML]{EEF8B3}\textcolor[HTML]{000000}{0.12}&\cellcolor[HTML]{73C8BD}\textcolor[HTML]{000000}{0.40}&\cellcolor[HTML]{93D5B9}\textcolor[HTML]{000000}{0.34} \\
gpt-4o-mini & n/a &\cellcolor[HTML]{C7E9B4}\textcolor[HTML]{000000}{0.25}&\cellcolor[HTML]{88D0BA}\textcolor[HTML]{000000}{0.36}&\cellcolor[HTML]{BBE5B5}\textcolor[HTML]{000000}{0.27}&\cellcolor[HTML]{F1F9B9}\textcolor[HTML]{000000}{0.10}&\cellcolor[HTML]{CDEBB4}\textcolor[HTML]{000000}{0.23} \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\textbf{Intervals.} Every rating carries a 95\% percentile bootstrap (\(10^3\)--\(10^4\) replicates). Units: \(G\) --- the per-(submission, archetype) units; \(f\) --- the \(N\) parallel contexts, each replicate's top-2 left subspace Procrustes-aligned to the full sample before the leading loading is read (the leading axis is near-degenerate between the Anthropic and Google blocs); \(\bar r\) --- the 55 (submission, archetype) atoms of the shared grid. \(E\) and \(T\) are \textbf{not} combined analytically: they are bootstrapped jointly on the 55-atom grid, leave-self-out applied after the resample, every component including \(f_a,\bar r_a\) recomputed on each replicate. The council is held at its selected membership rather than re-selected inside each replicate (the gate is itself defined by a bootstrap interval), so the interval is conditional on that selection.
...
\midrule\noalign{}
\ensuremath{\star} claude-opus-4.5 &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0} \\
claude-opus-4.1 &\cellcolor[HTML]{206EB0}\textcolor[HTML]{FFFFFF}{7.1}&\cellcolor[HTML]{1F7BB5}\textcolor[HTML]{FFFFFF}{6.8}&\cellcolor[HTML]{216AAE}\textcolor[HTML]{FFFFFF}{7.2}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2262AA}\textcolor[HTML]{FFFFFF}{7.4}&\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.0}&\cellcolor[HTML]{1F7FB7}\textcolor[HTML]{FFFFFF}{6.7}&\cellcolor[HTML]{1E87BB}\textcolor[HTML]{FFFFFF}{6.5}&\cellcolor[HTML]{2166AC}\textcolor[HTML]{FFFFFF}{7.3}&\cellcolor[HTML]{225BA6}\textcolor[HTML]{FFFFFF}{7.6} \\
claude-opus-4.0 &\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{6.9}&\cellcolor[HTML]{1F7FB7}\textcolor[HTML]{FFFFFF}{6.7}&\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{6.9}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.7}&\cellcolor[HTML]{1F7BB5}\textcolor[HTML]{FFFFFF}{6.8}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{2166AC}\textcolor[HTML]{FFFFFF}{7.3}&\cellcolor[HTML]{206EB0}\textcolor[HTML]{FFFFFF}{7.1} \\
claude-sonnet-4 &\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{2195C0}\textcolor[HTML]{FFFFFF}{6.1}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{1E8BBD}\textcolor[HTML]{FFFFFF}{6.4}&\cellcolor[HTML]{1E87BB}\textcolor[HTML]{FFFFFF}{6.5}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.2}&\cellcolor[HTML]{2195C0}\textcolor[HTML]{FFFFFF}{6.1}&\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.0}&\cellcolor[HTML]{1E8BBD}\textcolor[HTML]{FFFFFF}{6.4}&\cellcolor[HTML]{1F7FB7}\textcolor[HTML]{FFFFFF}{6.7} \\
gemini-3.1-pro &\cellcolor[HTML]{2A9EC1}\textcolor[HTML]{FFFFFF}{5.8}&\cellcolor[HTML]{1F7BB5}\textcolor[HTML]{FFFFFF}{6.8}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.7}&\cellcolor[HTML]{2262AA}\textcolor[HTML]{FFFFFF}{7.4}&\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.0}&\cellcolor[HTML]{2166AC}\textcolor[HTML]{FFFFFF}{7.3}&\cellcolor[HTML]{33A7C2}\textcolor[HTML]{FFFFFF}{5.5}&\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{6.9}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.7}&\cellcolor[HTML]{2262AA}\textcolor[HTML]{FFFFFF}{7.4} \\
gemini-2.5-flash &\cellcolor[HTML]{35AAC3}\textcolor[HTML]{FFFFFF}{5.4}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.2}&\cellcolor[HTML]{33A7C2}\textcolor[HTML]{FFFFFF}{5.5}&\cellcolor[HTML]{33A7C2}\textcolor[HTML]{FFFFFF}{5.5}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.2}&\cellcolor[HTML]{69C5BE}\textcolor[HTML]{000000}{4.2}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.7}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{2A9EC1}\textcolor[HTML]{FFFFFF}{5.8}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0} \\
gpt-4.1-mini &\cellcolor[HTML]{4BBAC3}\textcolor[HTML]{000000}{4.8}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{41B6C4}\textcolor[HTML]{000000}{5.0}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.2}&\cellcolor[HTML]{279BC1}\textcolor[HTML]{FFFFFF}{5.9}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{69C5BE}\textcolor[HTML]{000000}{4.2}&\cellcolor[HTML]{1E87BB}\textcolor[HTML]{FFFFFF}{6.5}&\cellcolor[HTML]{35AAC3}\textcolor[HTML]{FFFFFF}{5.4}&\cellcolor[HTML]{1F7FB7}\textcolor[HTML]{FFFFFF}{6.7} \\
gpt-4.1-2025-04-14 &\cellcolor[HTML]{3EB3C4}\textcolor[HTML]{000000}{5.1}&\cellcolor[HTML]{3BB0C3}\textcolor[HTML]{000000}{5.2}&\cellcolor[HTML]{41B6C4}\textcolor[HTML]{000000}{5.0}&\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.0}&\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.0}&\cellcolor[HTML]{1E92C0}\textcolor[HTML]{FFFFFF}{6.2}&\cellcolor[HTML]{88D0BA}\textcolor[HTML]{000000}{3.6}&\cellcolor[HTML]{1D8FBF}\textcolor[HTML]{FFFFFF}{6.3}&\cellcolor[HTML]{30A4C2}\textcolor[HTML]{FFFFFF}{5.6}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.0} \\
gpt-4.1-nano &\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.5}&\cellcolor[HTML]{3EB3C4}\textcolor[HTML]{000000}{5.1}&\cellcolor[HTML]{82CEBB}\textcolor[HTML]{000000}{3.7}&\cellcolor[HTML]{38ADC3}\textcolor[HTML]{000000}{5.3}&\cellcolor[HTML]{69C5BE}\textcolor[HTML]{000000}{4.2}&\cellcolor[HTML]{82CEBB}\textcolor[HTML]{000000}{3.7}&\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.5}&\cellcolor[HTML]{B6E2B6}\textcolor[HTML]{000000}{2.8}&\cellcolor[HTML]{9FD9B8}\textcolor[HTML]{000000}{3.2}&\cellcolor[HTML]{B0E0B6}\textcolor[HTML]{000000}{2.9} \\
gpt-4o-2024-08-06 &\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.5}&\cellcolor[HTML]{50BCC2}\textcolor[HTML]{000000}{4.7}&\cellcolor[HTML]{99D7B8}\textcolor[HTML]{000000}{3.3}&\cellcolor[HTML]{5ABFC0}\textcolor[HTML]{000000}{4.5}&\cellcolor[HTML]{6EC7BE}\textcolor[HTML]{000000}{4.1}&\cellcolor[HTML]{A4DCB7}\textcolor[HTML]{000000}{3.1}&\cellcolor[HTML]{99D7B8}\textcolor[HTML]{000000}{3.3}&\cellcolor[HTML]{73C8BD}\textcolor[HTML]{000000}{4.0}&\cellcolor[HTML]{AADEB7}\textcolor[HTML]{000000}{3.0}&\cellcolor[HTML]{2DA1C2}\textcolor[HTML]{FFFFFF}{5.7} \\
gpt-4o &\cellcolor[HTML]{93D5B9}\textcolor[HTML]{000000}{3.4}&\cellcolor[HTML]{C1E7B5}\textcolor[HTML]{000000}{2.6}&\cellcolor[HTML]{93D5B9}\textcolor[HTML]{000000}{3.4}&\cellcolor[HTML]{D6EFB3}\textcolor[HTML]{000000}{2.0}&\cellcolor[HTML]{50BCC2}\textcolor[HTML]{000000}{4.7}&\cellcolor[HTML]{F1F9B9}\textcolor[HTML]{000000}{1.0}&\cellcolor[HTML]{C1E7B5}\textcolor[HTML]{000000}{2.6}&\cellcolor[HTML]{A4DCB7}\textcolor[HTML]{000000}{3.1}&\cellcolor[HTML]{9FD9B8}\textcolor[HTML]{000000}{3.2}&\cellcolor[HTML]{B6E2B6}\textcolor[HTML]{000000}{2.8} \\
gpt-4o-mini &\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.5}&\cellcolor[HTML]{D3EEB3}\textcolor[HTML]{000000}{2.1}&\cellcolor[HTML]{8DD3BA}\textcolor[HTML]{000000}{3.5}&\cellcolor[HTML]{AADEB7}\textcolor[HTML]{000000}{3.0}&\cellcolor[HTML]{99D7B8}\textcolor[HTML]{000000}{3.3}&\cellcolor[HTML]{CDEBB4}\textcolor[HTML]{000000}{2.3}&\cellcolor[HTML]{7DCCBB}\textcolor[HTML]{000000}{3.8}&\cellcolor[HTML]{F3FBBF}\textcolor[HTML]{000000}{0.8}&\cellcolor[HTML]{AADEB7}\textcolor[HTML]{000000}{3.0}&\cellcolor[HTML]{D9F0B3}\textcolor[HTML]{000000}{1.9} \\
\textbf{cos(G,E)} &\cellcolor[HTML]{F2FABC}\textcolor[HTML]{000000}{\textbf{0.91}}& {[}.83,.95{]} &\cellcolor[HTML]{F2FABC}\textcolor[HTML]{000000}{\textbf{0.90}}& {[}.80,.93{]} &\cellcolor[HTML]{F2FABD}\textcolor[HTML]{000000}{\textbf{0.89}}& {[}.82,.92{]} &\cellcolor[HTML]{F3FABE}\textcolor[HTML]{000000}{\textbf{0.84}}& {[}.80,.87{]} &\cellcolor[HTML]{F2FABD}\textcolor[HTML]{000000}{\textbf{0.87}}& {[}.86,.87{]}  \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\midrule\noalign{}
gemini-3.1-pro &\cellcolor[HTML]{216BAE}\textcolor[HTML]{FFFFFF}{7.19}&\cellcolor[HTML]{2075B3}\textcolor[HTML]{FFFFFF}{6.94}&\cellcolor[HTML]{2353A3}\textcolor[HTML]{FFFFFF}{7.84}&\cellcolor[HTML]{24499E}\textcolor[HTML]{FFFFFF}{8.13}&\cellcolor[HTML]{24459C}\textcolor[HTML]{FFFFFF}{8.24} \\
\ensuremath{\star} claude-opus-4.5 &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00} \\
claude-opus-4.0 &\cellcolor[HTML]{2073B2}\textcolor[HTML]{FFFFFF}{6.98}&\cellcolor[HTML]{89D1BA}\textcolor[HTML]{000000}{3.58}&\cellcolor[HTML]{84CFBA}\textcolor[HTML]{000000}{3.66}&\cellcolor[HTML]{6CC6BE}\textcolor[HTML]{000000}{4.14}&\cellcolor[HTML]{50BCC2}\textcolor[HTML]{000000}{4.69} \\
claude-opus-4.1 &\cellcolor[HTML]{2261AA}\textcolor[HTML]{FFFFFF}{7.42}&\cellcolor[HTML]{66C4BF}\textcolor[HTML]{000000}{4.26}&\cellcolor[HTML]{7DCCBB}\textcolor[HTML]{000000}{3.79}&\cellcolor[HTML]{69C5BE}\textcolor[HTML]{000000}{4.20}&\cellcolor[HTML]{52BCC1}\textcolor[HTML]{000000}{4.65} \\
gemini-2.5-flash &\cellcolor[HTML]{E8F6B1}\textcolor[HTML]{000000}{1.41}&\cellcolor[HTML]{B9E3B5}\textcolor[HTML]{000000}{2.75}&\cellcolor[HTML]{6AC5BE}\textcolor[HTML]{000000}{4.18}&\cellcolor[HTML]{75C9BC}\textcolor[HTML]{000000}{3.95}&\cellcolor[HTML]{8FD3B9}\textcolor[HTML]{000000}{3.47} \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\begin{center}\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{0.14\linewidth}>{\raggedright\arraybackslash}p{0.40\linewidth}>{\raggedright\arraybackslash}p{0.44\linewidth}}
\toprule\noalign{}
...
\midrule\noalign{}
\(E^{C}\) alone &\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{0.81}&\cellcolor[HTML]{24469D}\textcolor[HTML]{FFFFFF}{0.82}& {[}0.45, 0.95{]} & {[}0.42, 0.94{]}  \\
\(G^{F}\) alone &\cellcolor[HTML]{22318D}\textcolor[HTML]{FFFFFF}{0.89}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}& {[}0.64, 0.97{]} & {[}0.72, 0.95{]}  \\
\(E^{F}\) alone &\cellcolor[HTML]{253695}\textcolor[HTML]{FFFFFF}{0.87}&\cellcolor[HTML]{0F2366}\textcolor[HTML]{FFFFFF}{0.97}& {[}0.60, 0.96{]} & {[}0.76, 0.93{]}  \\
\(G^{C}\) alone &\cellcolor[HTML]{142670}\textcolor[HTML]{FFFFFF}{0.95}&\cellcolor[HTML]{24409A}\textcolor[HTML]{FFFFFF}{0.84}& {[}0.82, 0.99{]} & {[}0.82, 0.98{]}  \\
\(G = \tfrac12(G^{F}+G^{C})\) --- generation half &\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{0.94}&\cellcolor[HTML]{253695}\textcolor[HTML]{FFFFFF}{0.87}& {[}0.81, 0.98{]} & {[}0.84, 0.98{]}  \\
\(E = \tfrac12(E^{F}+E^{C})\) --- evaluation half &\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{0.94}&\cellcolor[HTML]{142670}\textcolor[HTML]{FFFFFF}{0.95}& {[}0.80, 0.98{]} & {[}0.83, 0.98{]}  \\
\(\tfrac12(E^{F}+G^{F})\) --- the factual pair &\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{0.94}&\cellcolor[HTML]{11246B}\textcolor[HTML]{FFFFFF}{0.96}& {[}0.80, 0.98{]} & {[}0.89, 0.97{]}  \\
\(T = \tfrac14(G^{F}+G^{C}+E^{F}+E^{C})\) &\cellcolor[HTML]{0D2162}\textcolor[HTML]{FFFFFF}{\textbf{0.98}}&\cellcolor[HTML]{11246B}\textcolor[HTML]{FFFFFF}{\textbf{0.96}}& \textbf{{[}0.93, 1.00{]}} & \textbf{{[}0.95, 0.99{]}}  \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
\midrule\noalign{}
\(G^{F}\) &\cellcolor[HTML]{22318D}\textcolor[HTML]{FFFFFF}{0.89}&\cellcolor[HTML]{1F7FB7}\textcolor[HTML]{FFFFFF}{0.67} \\
\(G^{C}\) &\cellcolor[HTML]{142670}\textcolor[HTML]{FFFFFF}{\textbf{0.95}}&\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{0.81} \\
\(E^{F}\) &\cellcolor[HTML]{253695}\textcolor[HTML]{FFFFFF}{0.87}&\cellcolor[HTML]{22318D}\textcolor[HTML]{FFFFFF}{\textbf{0.89}} \\
\(E^{C}\) &\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{0.81}&\cellcolor[HTML]{82CEBB}\textcolor[HTML]{000000}{0.37} \\
\(\tfrac12(E^{F}+G^{F})\) &\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{0.94}&\cellcolor[HTML]{1B2C7E}\textcolor[HTML]{FFFFFF}{0.92} \\
\(\tfrac12(G^{C}+E^{F})\) &\cellcolor[HTML]{142670}\textcolor[HTML]{FFFFFF}{0.95}&\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{0.94} \\
\(T\) &\cellcolor[HTML]{0D2162}\textcolor[HTML]{FFFFFF}{\textbf{0.98}}&\cellcolor[HTML]{162875}\textcolor[HTML]{FFFFFF}{\textbf{0.94}} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\midrule\noalign{}
\ensuremath{\star} claude-opus-4.5 (anchor) &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2351A2}\textcolor[HTML]{FFFFFF}{78.79} \\
gemini-3.1-pro &\cellcolor[HTML]{2076B3}\textcolor[HTML]{FFFFFF}{6.92}&\cellcolor[HTML]{234A9F}\textcolor[HTML]{FFFFFF}{80.81} \\
claude-opus-4.1 &\cellcolor[HTML]{2498C1}\textcolor[HTML]{FFFFFF}{6.02}&\cellcolor[HTML]{2258A5}\textcolor[HTML]{FFFFFF}{76.77} \\
claude-opus-4.0 &\cellcolor[HTML]{2A9EC1}\textcolor[HTML]{FFFFFF}{5.81}&\cellcolor[HTML]{206DAF}\textcolor[HTML]{FFFFFF}{71.21} \\
gemini-2.5-flash &\cellcolor[HTML]{2BA0C2}\textcolor[HTML]{FFFFFF}{5.75}&\cellcolor[HTML]{2169AD}\textcolor[HTML]{FFFFFF}{72.22} \\
claude-sonnet-4 &\cellcolor[HTML]{37ACC3}\textcolor[HTML]{FFFFFF}{5.34}&\cellcolor[HTML]{2169AD}\textcolor[HTML]{FFFFFF}{72.22} \\
gpt-4.1-mini &\cellcolor[HTML]{40B5C4}\textcolor[HTML]{000000}{5.03}&\cellcolor[HTML]{1D8EBF}\textcolor[HTML]{FFFFFF}{63.13} \\
gpt-4.1-2025-04-14 &\cellcolor[HTML]{52BCC2}\textcolor[HTML]{000000}{4.66}&\cellcolor[HTML]{2094C0}\textcolor[HTML]{FFFFFF}{61.62} \\
gpt-4.1-nano &\cellcolor[HTML]{83CFBB}\textcolor[HTML]{000000}{3.68}&\cellcolor[HTML]{32A7C2}\textcolor[HTML]{FFFFFF}{55.05} \\
gpt-4o-2024-08-06 &\cellcolor[HTML]{97D6B9}\textcolor[HTML]{000000}{3.34}&\cellcolor[HTML]{53BDC1}\textcolor[HTML]{000000}{46.46} \\
gpt-4o &\cellcolor[HTML]{ADDFB7}\textcolor[HTML]{000000}{2.95}&\cellcolor[HTML]{49B9C3}\textcolor[HTML]{000000}{48.48} \\
gpt-4o-mini &\cellcolor[HTML]{CAEAB4}\textcolor[HTML]{000000}{2.39}&\cellcolor[HTML]{5FC1C0}\textcolor[HTML]{000000}{43.94} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

\section{The constructs of intelligence the game demands}\label{the-constructs-of-intelligence-the-game-demands}
...
\begin{table}[H]
\caption{The eight constructs mapped to the game's two tasks (\ensuremath{\bullet} marks a primary demand).}
...
\midrule\noalign{}
\ensuremath{\star} claude-opus-4.5 &\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{2072B2}\textcolor[HTML]{FFFFFF}{7.00}&\cellcolor[HTML]{FFFFD9}\textcolor[HTML]{000000}{0.00} \\
\ensuremath{\star} gemini-3.1-pro &\cellcolor[HTML]{1F81B8}\textcolor[HTML]{FFFFFF}{6.65}&\cellcolor[HTML]{2164AB}\textcolor[HTML]{FFFFFF}{7.35}&\cellcolor[HTML]{234A9E}\textcolor[HTML]{FFFFFF}{8.10}&\cellcolor[HTML]{216AAE}\textcolor[HTML]{FFFFFF}{0.72} \\
\ensuremath{\star} claude-opus-4.0 &\cellcolor[HTML]{2397C1}\textcolor[HTML]{FFFFFF}{6.04}&\cellcolor[HTML]{33A7C2}\textcolor[HTML]{FFFFFF}{5.50}&\cellcolor[HTML]{1D91C0}\textcolor[HTML]{FFFFFF}{6.26}&\cellcolor[HTML]{78CABC}\textcolor[HTML]{000000}{0.39} \\
\ensuremath{\star} gemini-2.5-flash &\cellcolor[HTML]{2398C1}\textcolor[HTML]{FFFFFF}{6.03}&\cellcolor[HTML]{299DC1}\textcolor[HTML]{FFFFFF}{5.84}&\cellcolor[HTML]{4CBAC2}\textcolor[HTML]{000000}{4.78}&\cellcolor[HTML]{1F7BB5}\textcolor[HTML]{FFFFFF}{0.68} \\
\ensuremath{\star} claude-opus-4.1 &\cellcolor[HTML]{279BC1}\textcolor[HTML]{FFFFFF}{5.92}&\cellcolor[HTML]{1F7CB6}\textcolor[HTML]{FFFFFF}{6.77}&\cellcolor[HTML]{1F93C0}\textcolor[HTML]{FFFFFF}{6.19}&\cellcolor[HTML]{64C3BF}\textcolor[HTML]{000000}{0.43} \\
claude-sonnet-4 &\cellcolor[HTML]{3BAFC3}\textcolor[HTML]{000000}{5.22}&\cellcolor[HTML]{299EC1}\textcolor[HTML]{FFFFFF}{5.82}&\cellcolor[HTML]{2094C0}\textcolor[HTML]{FFFFFF}{6.16}&\cellcolor[HTML]{50BCC2}\textcolor[HTML]{000000}{0.47} \\
gpt-4.1-mini &\cellcolor[HTML]{47B8C3}\textcolor[HTML]{000000}{4.88}&\cellcolor[HTML]{269AC1}\textcolor[HTML]{FFFFFF}{5.93}&\cellcolor[HTML]{44B7C3}\textcolor[HTML]{000000}{4.93}&\cellcolor[HTML]{279BC1}\textcolor[HTML]{FFFFFF}{0.59} \\
gpt-4.1-2025-04-14 &\cellcolor[HTML]{5DC0C0}\textcolor[HTML]{000000}{4.44}&\cellcolor[HTML]{54BDC1}\textcolor[HTML]{000000}{4.62}&\cellcolor[HTML]{2CA1C2}\textcolor[HTML]{FFFFFF}{5.72}&\cellcolor[HTML]{2076B4}\textcolor[HTML]{FFFFFF}{0.69} \\
gpt-4.1-nano &\cellcolor[HTML]{92D4B9}\textcolor[HTML]{000000}{3.42}&\cellcolor[HTML]{8BD1BA}\textcolor[HTML]{000000}{3.55}&\cellcolor[HTML]{BAE4B5}\textcolor[HTML]{000000}{2.72}&\cellcolor[HTML]{5ABFC0}\textcolor[HTML]{000000}{0.45} \\
gpt-4o-2024-08-06 &\cellcolor[HTML]{97D6B9}\textcolor[HTML]{000000}{3.34}&\cellcolor[HTML]{A6DCB7}\textcolor[HTML]{000000}{3.08}&\cellcolor[HTML]{86D0BA}\textcolor[HTML]{000000}{3.63}&\cellcolor[HTML]{BBE5B5}\textcolor[HTML]{000000}{0.27} \\
gpt-4o &\cellcolor[HTML]{B3E1B6}\textcolor[HTML]{000000}{2.85}&\cellcolor[HTML]{B8E3B5}\textcolor[HTML]{000000}{2.76}&\cellcolor[HTML]{AEDFB6}\textcolor[HTML]{000000}{2.93}&\cellcolor[HTML]{F2FABC}\textcolor[HTML]{000000}{0.09} \\
gpt-4o-mini &\cellcolor[HTML]{D3EEB3}\textcolor[HTML]{000000}{2.10}&\cellcolor[HTML]{CFECB3}\textcolor[HTML]{000000}{2.25}&\cellcolor[HTML]{E2F4B2}\textcolor[HTML]{000000}{1.60}&\cellcolor[HTML]{93D5B9}\textcolor[HTML]{000000}{0.34} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[width=1.00\linewidth]{figures/runs_panel.png}}
...
\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[width=0.60\linewidth]{figures/anchoring_resolution.png}}
...
\section{The argument behind the hypothesis}\label{the-argument-behind-the-hypothesis}

Nothing in this appendix tests the hypothesis of \S{}6. It sets out the argument step by step, marks each step as a cited result, a measurement on the released data, or the hypothesis itself, and ends with what would refute it. Every number comes from a script in the released package.

\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[width=0.32\linewidth]{figures/total_validation_simple.png}}
\caption{The agreement the hypothesis is offered for, reproduced from Figure 2: the official total \(T\) (council basis, three runs pooled) against self-administered GPQA Diamond accuracy, twelve models; Pearson \(r = 0.98\) (95\% BCa CI {[}0.95, 0.99{]}), Spearman \(\rho = 0.96\). Filled markers are council seats, open markers non-council, the star the anchor at \(T = 7\) by calibration.}
\label{fig-gpqa-scatter-h}
\end{figure}

\subsection{The chain}\label{the-chain}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Training a language model is compression} (cited). Predicting text is compressing it (Shannon, 1951); a trained model is a general-purpose compressor (Del\'{e}tang et al., 2024).
\item
  \textbf{Training selects what is compressed} (cited, by construction of the objective). The loss is taken on held-out text, so an abstraction earns its place only by predicting instantiations the model has not seen, and the abstractions that recur most across the corpus earn the most. A situation structure that appears in biology, economics and stories alike is one.
\item
  \textbf{Compression generalises} (cited). The shortest description keeps a recurring pattern once, as an abstraction, and reuses it (Rissanen, 1978); that is a concept.
\item
  \textbf{A concept is a frame with open slots} (cited). Minsky's (1974) frames are stereotyped situations with terminals that instances fill; the context template of \S{}2 is such a frame, written in words.
\item
  \textbf{An analogy is a shared relational structure with different particulars} (cited; Gentner, 1983). The archetypal context is that shared structure.
\item
  \textbf{Hypothesis.} A language model holds its knowledge as archetypal contexts, each instantiated in the topic domains where it applies. The template is the abstraction; the metanym set is what the abstraction leaves out, the topic domain; the two reconstruct the instantiation exactly.
\item
  \textbf{The metanym form is strong compression} (measured, H.2). About 3 for a template with five parallel contexts and about 8 for each further one, growing with every domain a template serves --- the order of the compression a trained model achieves on text, in words rather than bits. The factor grows because the template has open slots: a memorised string compresses only its exact repeats, a frame compresses every new instance that fits it, and no instantiation here repeats another. This is the abstracting kind of compression, the kind held-out loss rewards; a representation this compressive is what training would find.
\item
  \textbf{The game measures the mechanism directly} (hypothesis). Nothing is given; the model selects the archetype and the domains that best satisfy the prompt's criteria, factual truth among them, and writes them out.
\item
  \textbf{GPQA measures the mechanism's use} (hypothesis, with H.4 as the observation). The archetype and the domain are given by the question; the model derives the instantiation and selects the candidate that matches it. The two tests agree because they exercise one operation from opposite ends: GPQA fixes the store position and selects among four strings, the game selects the store position and fixes the output form.
\item
  \textbf{Corollary: retrieval, not construction} (hypothesis, with H.3 as the signature). In the game the models retrieve templates and metanym sets that training already compressed, which is why they play well with no reasoning channel and why a model rewrites its portfolios between runs but keeps its archetypes.
\item
  \textbf{What the data establish is narrower.} The factual pair reaches 0.94 with GPQA and the two subjective quarters lift it to 0.98 (\S{}4.5, Appendix D.1). The hypothesis is consistent with the general factor of Ili\'{c} and Gignac (2024), and more specific.
\end{enumerate}

\subsection{The compression of the metanym form}\label{the-compression-of-the-metanym-form}

Measured on the anchor submission's first archetype, the one archetype shipped in full (\texttt{scripts/compression\_\allowbreak{}ratio.py}). The template is 120 words; the metanym table 84 words for five parallel contexts, 17 per context; the five instantiations (Form a, the form the template and metanym set reconstruct exactly) average 130 words. With \(n\) contexts the factor is \(130n/(120+17n)\):

\begin{table}[H]
\caption{Literal compression of the metanym form against the number of parallel contexts per template, from the anchor's first archetype (120-word template, 17 words of metanyms per context, 130-word instantiations).}
\begin{center}\small
\resizebox{\linewidth}{!}{\begin{tabular}{@{}lrrrrr@{}}
\toprule\noalign{}
...
\subsection{The retrieval signature in the released transcripts}\label{the-retrieval-signature-in-the-released-transcripts}

The archetype titles and parallel-context domains of every graded portfolio in the three runs, as the evaluation transcripts record them (\texttt{scripts/archetype\_\allowbreak{}recurrence.py}; the raw portfolios are not shipped, the transcripts copy their headings).

\textbf{Within a model, the archetypes persist while the texts change.} Eleven of twelve portfolios differ between runs (\S{}4.6). Between the two runs made two hours apart, 34 of 48 archetype titles recur verbatim; between run 1 and run 2, three weeks apart, 20 of 50. Gemini 2.5 Flash produced the same five titles in all three runs; Claude Opus 4.0 kept four or five of five. The models rewrite; they do not reinvent. This is what one archetype in latent space with many projections in literal space predicts, and what a stored text would not.

\textbf{Across models, the same concepts recur, across vendors.} Seven of the eleven graded models, from all three vendors, offer a resource-allocation archetype (title containing ``allocation''). Six domains are used by five or more models: urban planning, social media, corporate structure, the climate system, software development, biological evolution. Of 81 distinct titles, 14 are used by more than one model.

\textbf{The caveat.} Recurrence is strongest inside a vendor: 11 of the 14 shared titles are shared within one vendor only, the two Opus models sharing cascade amplification, phase transition, competitive exclusion and boundary maintenance word for word, and the GPT-4o family sharing resource allocation, conflict resolution and growth and development. Shared training data explains within-vendor recurrence as well as shared representation does. The cross-vendor recurrence is the evidence.

\subsection{How GPQA was answered}\label{how-gpqa-was-answered}

With the reasoning channel off and temperature zero, no model answered with a bare letter (\texttt{scripts/gpqa\_\allowbreak{}reply\_\allowbreak{}lengths.py}):

\begin{table}[H]
\caption{Words per GPQA reply with the reasoning channel off, and replies under fifteen words, per model (198 items each).}
\begin{center}\small
\begin{tabular}{@{}lrr@{}}
\toprule\noalign{}
...
\subsection{Predictions, and what would refute the hypothesis}\label{predictions-and-what-would-refute-the-hypothesis}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Domain-matched agreement.} A model's factual score on parallel contexts in a field tracks its GPQA accuracy in that field. GPQA Diamond carries subject labels, and the released evaluations carry each parallel context's domain, so the test needs no new run. A refutation: the field-by-field agreement is no higher than the agreement across fields.
\item
  \textbf{Separability in latent space.} Archetype and topic domain are separable in a model's latent space: the same archetype recoverable from parallel contexts in unrelated domains, and the domain from unrelated archetypes. A refutation: representations that separate by domain only.
\item
  \textbf{Retrieval over construction.} Reasoning should add little to generation in the game, where templates and metanym sets are retrieved, and may still help judging, where the task is to catch a broken substitution. A refutation: a reasoning budget that raises generation ratings as much as it raises judging.
\item
  \textbf{The bit-level factor.} Given its template and metanym set, an instantiation's per-token loss under an open-weights model should fall far below its loss given nothing, and by more for archetypes that recur across many domains. A refutation: no such gap, or a gap that does not grow with the number of domains.
\end{enumerate}
