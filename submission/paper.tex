...
We ran the full pipeline three times, regenerating all twelve portfolios at T=0 against the frozen anchor; the main text pools the three. Apart, the runs agree on the total (Appendix F) and track GPQA at 0.97, 0.97 and 0.92, but not on the factual quarter: one seat's \(E^{F}\) reads 4.68, 3.81 and 0.01 across the runs, and run 3's factual axis is not identified (\(\sigma_1/\sigma_2 = 1.29\); run 2 1.67, run 1 2.57). Re-selected from a single run, the council would change one seat in run 2 and one in run 3; neither rotation clears the guard of \S{}3.4. Pooled, the axis is identified (\(\sigma_1/\sigma_2 = 2.2\)) and the guard holds in every contest (Appendix A.6).
