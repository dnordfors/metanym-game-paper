...
\textbf{Same-vendor robustness.} The fair worry is that a Claude-heavy evaluator set reads Claude-bloc agreement as truth. It does not: recomputing \(G^{F}\) with each vendor's judges removed leaves the ordering essentially unchanged (Spearman \ensuremath{\ge} 0.94 for every reduced set), gpt-4o-mini stays at the floor under every evaluator set, and a \emph{Claude-free} set (Google + OpenAI judges) still places the Claude generators at the top (\ensuremath{\ge} 7.0).
