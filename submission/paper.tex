...
The test is the \textbf{metanym game}. A player authors, from nothing, a \emph{context template} --- a paragraph of fixed wording with open slots --- together with the sets of keywords that fill it, each set instantiating the template as a factually true description of a different domain; the keywords in corresponding slots are \emph{metanyms}, metaphorically synonymous, and a set of them a \emph{metanym set}. Table \ref{tab-anchor-metanym} shows one, written by a player. Its instantiations are each other's \emph{metaphors} and, as a set, \emph{parallel contexts}: children of a common \emph{archetypal context}, the abstract structure they share, of which the template is the literal representation. A long tradition treats seeing one structure across wildly different domains as central to thought and tests whether you \emph{recognise} it; the game tests whether you can \emph{build} it.
...
\begin{table}[htb]
\caption{The anchor's first archetype (\S{}4.1), as a player wrote it: the context template, its metanym table, and its first column played, first sentence of each form.}
...
\end{tabular}}
\end{center}
\vspace{4pt}\begin{flushleft}\footnotesize Each parallel context is played in two forms: the \textbf{instantiation} (Form a), the mechanical substitution --- only the slots filled, every other word carried over --- the form the factual criterion is written for, since it must come out true sentence by sentence (the judge sees both forms); and the \textbf{idiomatic rewrite} (Form b) in the target domain's own register, showing the claim is not an artefact of the template's phrasing.\end{flushleft}
\begin{center}\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{0.21\linewidth}>{\raggedright\arraybackslash}p{0.71\linewidth}}
\toprule\noalign{}
...
\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[width=1.00\linewidth]{figures/council_evaluation_pc1.png}}
...
\section{A hypothesis for the metanym game--GPQA correlation}\label{a-hypothesis-for-the-metanym-gamegpqa-correlation}
...
\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[width=1.00\linewidth]{figures/mechanism_sketch.png}}
...
\tightlist
\item
  \textbf{Cited.} Training a language model is compression. Predicting text is compressing it (Shannon, 1951); a trained model is a general-purpose compressor (Del\'{e}tang et al., 2024).
\item
  \textbf{Cited.} Training selects what is compressed, by construction of the objective. The loss is taken on held-out text, so an abstraction earns its place only by predicting instantiations the model has not seen, and the abstractions that recur most across the corpus earn the most. A situation structure that appears in biology, economics and stories alike is one.
\item
  \textbf{Cited.} Compression generalises. The shortest description keeps a recurring pattern once, as an abstraction, and reuses it (Rissanen, 1978); that is a concept.
\item
  \textbf{Cited.} A concept is a frame with open slots. Minsky's (1974) frames are stereotyped situations with terminals that instances fill; the context template of \S{}2 is such a frame, written in words.
\item
  \textbf{Cited.} An analogy is a shared relational structure with different particulars (Gentner, 1983). The archetypal context is that shared structure.
\item
...
\item
  \textbf{Measured (H.2).} The metanym form is strong compression: an instantiation is five times its metanym set, so the form converges toward a factor of five as domains are added, counted in words, the order of a language model's own compression of text. The factor grows with the domains because the template has open slots: a memorised string compresses only its exact repeats, a frame compresses every new instance that fits it.
\item
  \textbf{Hypothesis.} The game measures the mechanism directly. Nothing is given; the model selects the archetype and the domains that best satisfy the prompt's criteria, factual truth among them, and writes them out.
\item
  \textbf{Hypothesis (H.4 is the observation).} GPQA measures the mechanism's use. The archetype and the domain are given by the question; the model derives the instantiation and selects the candidate that matches it. The two tests agree because they exercise one operation from opposite ends: GPQA fixes the store position and selects among four strings, the game selects the store position and fixes the output form.
\item
  \textbf{Hypothesis (H.3 is the signature).} Corollary: retrieval, not construction. In the game the models retrieve templates and metanym sets that training already compressed, which is why they play well with no reasoning channel and why a model rewrites its portfolios between runs but keeps its archetypes.
\item
  \textbf{Established.} What the data establish is narrower. The factual pair reaches 0.94 with GPQA and the two subjective quarters lift it to 0.98 (\S{}4.5, Appendix D.1). The hypothesis is consistent with the general factor of Ili\'{c} and Gignac (2024), and more specific.
\end{enumerate}

\subsection{The compression of the metanym form}\label{the-compression-of-the-metanym-form}

The average instantiation in the metanym game is five times larger than its metanym set (median 4.8 over the 105 archetypes in the run 2 and 3 portfolios, range 2.6 to 10.7; the anchor's first archetype 7.8), so the compression of the metanym form converges toward a factor of five as domains are added, from 2.6 at the five contexts of one game. Counted in words, that is the order of the compression a language model achieves on text, which Del\'{e}tang et al.~(2024) measured at 8.3\% of raw size for Chinchilla 70B on Wikipedia, a factor of twelve, against three for gzip; the bit-level factor of the metanym form is measurable on an open-weights model and is not measured here.

\subsection{The retrieval signature: the runs part on one word, and the archetypes return}\label{the-retrieval-signature-the-runs-part-on-one-word-and-the-archetypes-return}

At temperature 0 the decoder takes the highest-scoring token at each position. Serving is not bit-exact from call to call, and where two candidates score almost equally a difference below rounding decides between them; from that token on the text is generated anew. Runs 2 and 3 were made two hours apart from the same prompt at temperature 0, and the raw portfolios of both are shipped (\texttt{data/regenerations/portfolios\_\allowbreak{}run2/}, \texttt{portfolios\_\allowbreak{}run3/}; \texttt{scripts/portfolio\_\allowbreak{}divergence.py}). Run 1's portfolios were not preserved, so the three-week pair is read from the evaluation transcripts, which copy the headings (\texttt{scripts/archetype\_\allowbreak{}recurrence.py}).

\textbf{Where the runs part.} Gemini 2.5 Flash returned the same 9,099 words. The other eleven models part inside the first template sentence, after 3 to 93 shared words, on one word: \emph{risks} against \emph{faces}, \emph{that detect} against \emph{to detect}, \emph{activates} against \emph{encounters}. Everything after the fork, two to eight thousand words, is generated anew. After the fork, 22 of 42 archetypes return by title; in the four models that keep the most, Claude Opus 4.0, GPT-4.1, Gemini 3.1 Pro and GPT-4o, 14 of 17.

\begin{table}[H]
\caption{Where the raw portfolios of runs 2 and 3 part (same prompt, temperature 0, two hours apart), and how many of run 2's archetypes after the fork return in run 3 by title. gpt-4o-mini's run 2 portfolio holds one archetype, the one the fork falls in.}
\begin{center}\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.08\linewidth}>{\raggedright\arraybackslash}p{0.44\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}}
\toprule\noalign{}
Model & Words shared & The fork (run 2 / run 3) & Archetypes returning after it \\
\midrule\noalign{}
claude-opus-4.0 & 23 & \emph{activated {[}UNIT{]} recruits} / \emph{{[}UNIT{]} that receives} & 4 / 4 \\
claude-opus-4.1 & 14 & \emph{activates a {[}DETECTOR{]}} / \emph{encounters a {[}DETECTOR{]}} & 1 / 4 \\
claude-opus-4.5 & 40 & \emph{that detect changes} / \emph{to detect changes} & 2 / 5 \\
claude-sonnet-4 & 15 & \emph{{[}ORGANIZATION{]}. Each} / \emph{{[}ORGANIZATION{]} while} & 2 / 4 \\
gemini-2.5-flash & 9,099 & identical & 6 / 6 \\
gemini-3.1-pro & 93 & \emph{risks {[}CONSEQUENCE{]}} / \emph{faces {[}CONSEQUENCE{]}} & 3 / 4 \\
gpt-4.1 & 75 & first table cell: \emph{Biological Organism} / \emph{Genetics} & 4 / 5 \\
gpt-4.1-mini & 3 & \emph{{[}Resource Flow} / \emph{Resource Flow} & 1 / 5 \\
gpt-4.1-nano & 14 & \emph{based on {[}INFORMATION{]}} / \emph{by a {[}DECISION-MAKER{]}} & 1 / 3 \\
gpt-4o & 23 & \emph{{[}CRITERIA{]} to ensure} / \emph{{[}STRATEGY{]} to ensure} & 3 / 4 \\
gpt-4o-2024-08-06 & 23 & \emph{{[}CRITERIA{]} to ensure} / \emph{balancing {[}CONSTRAINT{]}} & 1 / 4 \\
gpt-4o-mini & 26 & \emph{relative importance of} / \emph{value of each} & 0 / 0 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

\textbf{What returns and what does not.} Of the 37 archetypes a model kept by title, 30 templates are rewritten, with a median text similarity of 0.19; half the slot names are kept (199 of 387), a third of the domains (53 of 150), and where a domain and a slot are both kept the metanym is the same in 221 of 388 cells. Seven templates return verbatim: Gemini 2.5 Flash's six, and one GPT-4.1 template returned word for word with all five domains replaced, the fork falling on the first cell of its metanym table.

\textbf{The reading.} The perturbation acts in literal space: the first near-degenerate logits after the prompt decide the wording, and from there the context template and the metanym sets are written anew. What returns is the archetype, in a different literal representation. This is what one archetype in latent space with many projections in literal space predicts. A stored text would return verbatim or not at all; Gemini 2.5 Flash's identical portfolio is the one case that does not separate the two. Between run 1 and run 2, three weeks apart, 20 of 50 titles recur.
...
\item
  \textbf{Recurrence under a changed prompt.} With the two worked examples of the generation prompt replaced, the same archetypes should return, rewritten as in H.3. A refutation: recurrence that vanishes with the examples, which would put the archetypes in the prompt and not in the model.
\end{enumerate}
